...
mdbSrcDir='/usr/local/src/mariadb'
...
if [ ! -d $mdbSrcDir ]; then
  mkdir -p $mdbSrcDir && cd $mdbSrcDir
else cd $mdbSrcDir
fi

if [ ! -f $mdbVer.tar.gz ] && [ ! -d $mdbVer ]; then
  # Nginx version nginx-1.15.5
    $wget -O $mdbVer.tar.gz https://downloads.mariadb.org/interstitial/$mdbVer/source/$mdbVer.tar.gz/from/http%3A//download.nus.edu.sg/mirror/mariadb/ && tar -zxf $mdbVer.tar.gz
elif [ ! -d $mdbVer ]; then
  tar -zxf $mdbVer.tar.gz
else echo "File name $nginxVer already exists"
fi
